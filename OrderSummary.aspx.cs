...
public partial class OrderSummary : BaseProcessCheckoutPage
{
    #region Private
    ...
    {
        string errorMessage = "<p>The following recurring products cannot use coupon discount and GiftCertificate</p>";

        foreach (ICartItem item in cart)
        {
            if (item.IsRecurring)
            {
                errorMessage += "<li> " + item.GetName( StoreContext.Culture, StoreContext.Currency ) + "</li>";
            }
        }
    ...
    {
        string errorMessage = "<p>The following recurring products cannot use GiftCertificate</p>";
    ...
    private void ShowQuantityDiscountNoCouponWarningMessage( ICartItem[] cart )
    {
        ...
        {
            errorMessage = "<p>The following products cannot use coupon discount since they " +
        ...
        {
            errorMessage = "<p>The following recurring products cannot use coupon discount</p>";
    ...
    private void ShowPartialCouponMessage()
    {
        if (GetCouponDiscount() > 0)
        {
            ...
            {
                string errorMessage = "<p>Coupon will not apply to all products in your shopping cart.</p>" +
                ...
                for (int i = 0; i < cart.Length; i++)
                {
                    ...
                    {
                        errorMessage += "<li>" + cart[i].GetName( StoreContext.Culture, StoreContext.Currency ) + "</li>";
                ...
                uxMessageLiteral.DisplayMessage( errorMessage );
            }
        }
    }
    ...
    protected void Page_PreRender( object sender, EventArgs e )
    {
        ...
        uxPointDiscountLabel.Text = StoreContext.Currency.FormatPrice( orderAmount.PointDiscount * -1 );
    ...
    protected bool DisplayPointEarned()
    {
        ...
        {
            if (StoreContext.CheckoutDetails.PaymentMethod is AnonymousPaymentMethod)
                return false;

            if (Membership.GetUser() == null)
                return false;

            if (StoreContext.CheckoutDetails.RedeemPoint > 0 || StoreContext.CheckoutDetails.RedeemPrice > 0)
                return false;
