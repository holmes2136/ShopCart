...
public partial class Layouts_CategoryLists_CategoryListDefault2 : BaseCategoryListControl
{
    ...
    private void PopulateCategoryImageControl()
    {
        Category category = DataAccessContext.CategoryRepository.GetOne( StoreContext.Culture, GetCategoryID() );

        uxCatalogImage.ImageUrl = category.ImageFile;
        uxCatalogImage.AlternateText = category.ImageAlt;
        uxCatalogImage.Title = category.ImageTitle;

        if ( !String.IsNullOrEmpty( category.Description ) )
            uxCatalogDescription.Text = category.Description;
        else
            uxCatalogDescription.Text = category.Name;
    }
    ...
    public void RefreshWithoutCategory()
    {
        if ( !IsFacetedSearch() )
        {
            if ( DataAccessContext.Configurations.GetBoolValue( "CategoryShowProductList", new StoreRetriever().GetStore() ) )
