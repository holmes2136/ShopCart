<%@ Control Language="C#" AutoEventWireup="true" CodeFile="DepartmentList.ascx.cs"
    Inherits="Admin_MainControls_DepartmentList" %>
    ...
    <GridTemplate>
        <asp:GridView ID="uxGrid" runat="server" CssClass="Gridview1" SkinID="DefaultGridView"
            ...
            <Columns>
                ...
                <asp:TemplateField HeaderText="<%$ Resources:DepartmentFields, Name %>">
                    <ItemTemplate>
                        <div id="uxSpaceDiv" runat="server" style='<%# GetSpaceStyle( Eval("level") ) %>'>
                            &nbsp;
                        </div>
                        <asp:Image ID="uxDepartmentImage" runat="server" SkinID="SmallDepartment" />
                        <asp:Label ID="uxNameLable" runat="server" Text='<%# GetName(Eval("Name").ToString(), Eval("DepartmentID").ToString(), Eval("Level").ToString()) %>'>
                        </asp:Label>
                ...
                <asp:TemplateField HeaderText="<%$ Resources:DepartmentFields, IsEnabled %>">
                    <ItemTemplate>
                ...
                <asp:TemplateField HeaderText="<%$ Resources:DepartmentFields, EditCommand %>">
                    <ItemTemplate>
                        <vevo:AdvancedLinkButton ID="uxEditHyperLink" runat="server" ToolTip="<%$ Resources:DepartmentFields, EditCommand %>"
                            PageName="DepartmentEdit.ascx" PageQueryString='<%# PageQueryString( Eval("DepartmentID").ToString() ) %>'
                            OnClick="ChangePage_Click" SkinID="EditLinkButton" StatusBarText='<%# String.Format("Department {0} Edit",Eval("Name") ) %>'>
