...
public partial class Layouts_ProductDetails_ProductDetailsDefaultBlue : BaseProductDetails
{
    #region Private
    ...
    protected string GetProductDimension( object productID )
    {
        ...
        if (product.Length > 0 || product.Width > 0 || product.Height > 0)
        {
            ...
            if (DataAccessContext.Configurations.GetValue( "WeightUnit" ).ToLower().Equals( "lb" ))
            {
                linearUnit = "in";
            }
            else
            ...
            dimension = product.Length.ToString() + " x " + product.Width.ToString() + " x " + product.Height.ToString() + " " + linearUnit;
        }
        else
        {
            uxDimensionTR.Visible = false;
        }
